...
# --------------------------- load demos data ----------------------------------------
data = sio.loadmat('../cartpole/data/cartpole_demos.mat')
...
KKT_loss = 0
parameter_theta = cartpoleoc.auxvar
...
for i in range(n_demo):
    ...
    for t in range(0, horizon - 1):
        ...
        loss_curr_1 = dot(LAM - cartpoleoc.dHx_fn(x_curr, u_curr, LAM_next, parameter_theta),
                          LAM - cartpoleoc.dHx_fn(x_curr, u_curr, LAM_next, parameter_theta))
        loss_curr_2 = dot(cartpoleoc.dHu_fn(x_curr, u_curr, LAM_next, parameter_theta),
                          cartpoleoc.dHu_fn(x_curr, u_curr, LAM_next, parameter_theta))
        KKT_loss = KKT_loss + loss_curr_1 + loss_curr_2
        # update
        LAM = LAM_next
    # final cost term
    KKT_loss = KKT_loss + dot(LAM - cartpoleoc.dhx_fn(state_traj[-1, :], parameter_theta),
                              LAM - cartpoleoc.dhx_fn(state_traj[-1, :], parameter_theta))
...
for j in range(10):  # loop for trials
    ...
    for k in range(int(1e4)):  # loop for iteration
        ...
        for i in range(n_demo):
            ...
            current_traj = cartpoleoc.ocSolver(initial_state, horizon, current_theta)
