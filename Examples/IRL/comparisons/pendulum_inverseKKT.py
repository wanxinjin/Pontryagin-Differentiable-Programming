...
# --------------------------- load demos data ----------------------------------------
data = sio.loadmat('../pendulum/data/pendulum_demos.mat')
...
KKT_loss = 0
parameter_theta = pendulumoc.auxvar
...
for i in range(n_demo):
    ...
    for t in range(0, horizon - 1):
        ...
        loss_curr_1 = dot(LAM - pendulumoc.dHx_fn(x_curr, u_curr, LAM_next, parameter_theta),
                          LAM - pendulumoc.dHx_fn(x_curr, u_curr, LAM_next, parameter_theta))
        loss_curr_2 = dot(pendulumoc.dHu_fn(x_curr, u_curr, LAM_next, parameter_theta),
                          pendulumoc.dHu_fn(x_curr, u_curr, LAM_next, parameter_theta))
        KKT_loss = KKT_loss + loss_curr_1 + loss_curr_2
        # update
        LAM = LAM_next
    # final cost term
    KKT_loss = KKT_loss + dot(LAM - pendulumoc.dhx_fn(state_traj[-1, :], parameter_theta),
                              LAM - pendulumoc.dhx_fn(state_traj[-1, :], parameter_theta))
...
for j in range(10):  # loop for trials
    ...
    initial_parameter = initial_parameter + sigma * np.random.randn(len(initial_parameter))
    current_parameter = initial_parameter

    for k in range(int(2e5)):  # loop for iteration
        ...
        kkt_loss_trace += [KKT_loss_fn(current_parameter).full().flatten().item()]
        ...
        for i in range(n_demo):
            ...
            current_traj = pendulumoc.ocSolver(initial_state, horizon, current_theta)
