...
for j in range(6):  # trial loop
    ...
    for k in range(int(5000)):  # iteration loop (or epoch loop)
        loss = 0
        ...
        for i in range(n_demo):
            ...
            traj = rocketoc.ocSolver(demo_ini_state, demo_horizon, current_parameter)
            # Establish the auxiliary control system
            aux_sys = rocketoc.getAuxSys(state_traj_opt=traj['state_traj_opt'],
            ...
            aux_sol = lqr_solver.lqrSolver(numpy.zeros((rocketoc.n_state, rocketoc.n_auxvar)), demo_horizon)
