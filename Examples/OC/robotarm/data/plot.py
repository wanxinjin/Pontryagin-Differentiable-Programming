...
for i in range(5):
    ...
    loss_trace = load['results']['loss_trace'][0, 0].flatten()
    # if i==3:
    #     loss_trace=loss_trace[200:]
    gps_loss_list += [loss_trace]


# plot  results
for pdp_neural_loss, gps_loss in zip( pdp_neural_loss_list,gps_loss_list):
    ax.plot(pdp_neural_loss, marker='P', markevery=50, color=[0.6350, 0.0780, 0.1840], linewidth=4,markersize=10)
    ax.plot(gps_loss, marker='o', markevery=50, color = '#77AC30', linewidth=4,markersize=10,)

# show legend
line_gps,=ax.plot(gps_loss_list[0], marker='o', markevery=200, color = '#77AC30', linewidth=4,markersize=10)
